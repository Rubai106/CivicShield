...
export default function EditDraft() {
  ...
  console.log('EditDraft: Component loaded, draft ID:', id);
  ...
  useEffect(() => {
    ...
    // Load draft data
    console.log('EditDraft: Loading draft data for ID:', id);
    reportsAPI
      .getById(id)
      .then(({ data }) => {
        console.log('EditDraft: Draft data received', data);
        const report = data.data.report;
        if (!report.is_draft) {
          toast.error('This is not a draft');
          navigate('/dashboard');
          return;
        }

        console.log('EditDraft: Setting form data', {
          title: report.title,
        ...
        setForm({
          ...
          incident_time: report.incident_date ? new Date(report.incident_date).toTimeString().slice(0, 5) : '',
        });

        setExistingFiles(data.data.evidence || []);
      })
      .catch((err) => {
        console.error('EditDraft: Failed to load draft', err);
  ...
  // Add debugging to detect auto-form submission
  useEffect(() => {
    const timer = setTimeout(() => {
      console.log('EditDraft: Checking for unexpected form submission...');
    }, 3000);
  ...
  const saveDraft = async () => {
    ...
    console.log('EditDraft: saveDraft called, should save as draft');
    saveReport(true);
  };
  ...
  const handleSubmit = async (e) => {
    if (isSubmitting) return;
    setIsSubmitting(true);
    console.log('EditDraft: Form submitted, asDraft:', false);
    e.preventDefault();
    saveReport(false);
  };

  const saveReport = async (asDraft) => {
    console.log('EditDraft: saveReport called, asDraft:', asDraft);
    ...
    try {
      ...
      console.log('EditDraft: FormData is_draft set to:', asDraft ? 'true' : 'false');
      ...
      // Include existing file IDs to keep
      const existingFileIds = existingFiles.map(f => f.id);
      fd.append('existing_file_ids', JSON.stringify(existingFileIds));

      const { data } = await reportsAPI.update(id, fd);
      const report = data.data.report;
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="max-w-3xl mx-auto px-4 py-8 page-enter">
        <h1 className="text-2xl font-semibold text-slate-100 mb-2">Edit Draft</h1>
        <p className="text-slate-400 text-sm mb-6">Update your draft or submit it as a report.</p>
          ...
          <div>
            ...
            {existingFiles.length > 0 && (
              <div className="mb-3 space-y-2">
                <p className="text-xs text-slate-400">Existing files:</p>
                {existingFiles.map((file) => (
                  <div key={file.id} className="flex items-center justify-between bg-white/[0.03] border border-white/[0.08] rounded p-2">
                    <div>
                      <p className="text-sm text-slate-200">{file.file_name || 'Evidence file'}</p>
                      <p className="text-xs text-slate-500">{file.file_type || 'unknown type'}</p>
                    </div>
                    <button
                      type="button"
            ...
            {files.length > 0 && (
              <div className="mt-3 space-y-2">
                <p className="text-xs text-slate-400">New files:</p>
                {files.map((file, idx) => (
